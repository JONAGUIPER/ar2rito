...
class MyEventHandler extends \danog\MadelineProto\EventHandler
{
     ...
     */
    const ADMIN = 'jonaguiper'; // Change this
    
    /**
     * @var string directorio para descargas
     */
    const DIR_DOWNLOADS = '/descargas';
    ...
    public function onUpdateNewMessage(array $update): \Generator
    {
        ...
        try {
            ...
            //si me pasan un archivo o un media...
            if (isset($update['message']['media']['_']) && $update['message']['media']['_'] !== 'messageMediaWebPage') {
                ...
                if ($peerId != $this->adminId) {
                    yield $this->messages->sendMessage([
                        'peer' => $peerId, 
                        'message' => 'No es el administrador del bot para usar este bot ', 
                        'reply_to_msg_id' => $messageId]);
                    return;
                }else{
            ...
            if (!\file_exists(self::DIR_DOWNLOADS.'/'.$url["_"])) {
                mkdir(self::DIR_DOWNLOADS.'/'.$url["_"], 0777, true);
            }
            //descargar fichero a disco
            $output_file_name = yield $this->downloadToDir(
                $url,
                new \danog\MadelineProto\FileCallback(
                    self::DIR_DOWNLOADS.'/'.$url["_"],
                    function ($progress, $speed, $time) use ($peerId, $id) {
                        $this->logger("Progreso de: $progress%");
...
$MadelineProto = new \danog\MadelineProto\API(/*($argv[1] ?? 'bot').*/'ar2rito.madeline', $settings);
